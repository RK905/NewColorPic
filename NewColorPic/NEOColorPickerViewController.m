...
#define kPadding 8
  ...
  for (int i = 0; i < colorCount && i < _colorArray.count; i++) {
    ...
    layer.frame = CGRectMake(kPadding + ((CGFloat)column * (buttnWidth + kPadding)), kPadding + row * (40 + kPadding), buttnWidth, 40);
    //layer.frame = CGRectMake(i * viewWidth + kPadding + (column * (buttnWidth + kPadding)), kPadding + row * (40 + kPadding), 70, 40);
...
- (void) colorGridTapped:(UITapGestureRecognizer *)recognizer {
    ...
    CGFloat butnWidth = self.view.frame.size.width / 4;
  
    int row = (int)((point.y - 8) / 48);
    int column = (int)((point.x - 8) / butnWidth);
