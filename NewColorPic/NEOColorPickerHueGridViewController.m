...
- (void)viewDidLoad {
  ...
  CGRect lclScreenFrm   = [self.view convertRect:screenFrame toView:nil];
  CGFloat viewWidth = lclScreenFrm.size.width;
  ...
  for (int i = 0; i < 12; i++) {
    ...
    for (int x = 0; x < colorCount && index < self.hueColors.count; x++) {
      ...
      layer.frame = CGRectMake(i * viewWidth + kPadding + (column * (buttnWidth + kPadding)), kPadding + row * (40 + kPadding), buttnWidth, 40);
      [self setupShadow:layer];
      [self.scrollView.layer addSublayer:layer];
    }
  }
    ...
    self.scrollView.contentSize = CGSizeMake(12 * viewWidth, 296);
...
- (void) colorGridTapped:(UITapGestureRecognizer *)recognizer {
  ...
  CGFloat buttnWidth = viewWidth/4 - 2*kPadding;
  
  int row = (int)((point.y - 8) / 48);
  int column = (int)((delta - 8) / (buttnWidth + kPadding));
  int colorCount = NEOColorPicker4InchDisplay() ? 32 : 24;
...
- (void) colorBarTapped:(UITapGestureRecognizer *)recognizer {
  ...
  int page = point.x / (self.colorBar.frame.size.width/12.0);
  
  [self.scrollView scrollRectToVisible:CGRectMake(page*viewWidth, 0, viewWidth, self.scrollView.frame.size.height) animated:YES];
}
